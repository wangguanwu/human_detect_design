...
while True:
    ...
    if bgframe_cnt >= th_bgframes:
        ...
        inter_result = cubicInterpolate(points,average_temp
                ,grid_x,grid_y,'cubic')
        ...
        ax1.set_ylabel('Y')
        ax1.axis([0,31,32,0])
        ax2.hist(np.array(inter_result).ravel(),bins=256,range=(19,28),fc='k',ec='k')
        
        ax2.set_xlabel('temperature')
        ax2.set_ylabel('%')
        temp_diff = np.array(temp) - inter_result
        ax3.hist(temp_diff.ravel(),bins = 256,range = (-1.0,0.9),fc='k',ec='k')
